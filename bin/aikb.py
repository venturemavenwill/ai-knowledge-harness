...
# Routing tables are generated from manifest data so they cannot drift from the
# namespace set. Only the marked block is generated; the surrounding prose in
# each surface stays hand-written.
ROUTING_BEGIN = "<!-- BEGIN aikb-routing -->"
ROUTING_END = "<!-- END aikb-routing -->"
ROUTING_NOTE = "<!-- Generated by `aikb refresh`. Do not hand-edit this block. -->"
# Generated order is deterministic but not alphabetical: a reader works down the
# table, so guards that apply before every action come first, then repair and
# verification, then domain knowledge, then knowledge about the harness itself.
# An unrecognized root sorts last. Within a group, ordering is alphabetical, so
# a specialization always follows the parent whose name it extends.
ROUTING_ROOT_ORDER = ("guard", "engineering", "reasoning", "retrieval", "knowledge")
SURFACE_ROUTING_TARGETS = (
...
def _validate_manifest(
    ...
    _required_keys(
        data,
        required,
        required | {"$schema", "routing_summary", "capability_summary"},
        label,
        errors,
    )
    ...
    for field, limit in (("routing_summary", 120), ("capability_summary", 200)):
        ...
        if not isinstance(value, str) or not value.strip():
            ...
            errors.append(f"{label}.{field}: must be at most {limit} characters")
        elif value != value.strip():
            errors.append(f"{label}.{field}: must not have leading or trailing whitespace")
        elif "|" in value or "\n" in value:
            errors.append(f"{label}.{field}: must not contain '|' or a line break")
...
def _load_state(repo: Path) -> tuple[RepositoryState | None, list[str]]:
    ...
    for namespace_root in sorted(
        ...
        # Routing tables are generated from the active manifest, so a routable
        # namespace must carry the fields those tables are rendered from.
        if active.data.get("consult_when"):
            for field in ("routing_summary", "capability_summary"):
                if not active.data.get(field):
                    errors.append(
                        f"{_relative(repo, active.path)}.{field}: required because "
                        "consult_when is declared; the routing tables are generated from it"
                    )
...
def _routing_sort_key(namespace: str) -> tuple[int, str]:
    root = namespace.split(".", 1)[0]
    try:
        rank = ROUTING_ROOT_ORDER.index(root)
    except ValueError:
        rank = len(ROUTING_ROOT_ORDER)
    return rank, namespace


def _routing_rows(state: RepositoryState) -> list[tuple[str, str, str]]:
    """Routable namespaces with the labels their tables are rendered from."""
    rows: list[tuple[str, str, str]] = []
    for record in sorted(state.namespaces, key=lambda item: _routing_sort_key(item.namespace)):
        data = record.active.data
        if not data.get("consult_when"):
            continue
        rows.append(
            (
                record.namespace,
                str(data.get("routing_summary", "")),
                str(data.get("capability_summary", "")),
            )
        )
    return rows


def _render_routing_block(rows: Sequence[tuple[str, str, str]], style: str) -> str:
    lines = [ROUTING_BEGIN, ROUTING_NOTE, ""]
    if style == "routing":
        lines.extend(("| Situation | Namespace |", "|---|---|"))
        lines.extend(f"| {routing} | `{namespace}` |" for namespace, routing, _ in rows)
    else:
        lines.extend(("| Namespace | What it adds |", "|---|---|"))
        lines.extend(f"| `{namespace}` | {capability} |" for namespace, _, capability in rows)
    lines.extend(("", ROUTING_END))
    return "\n".join(lines)


def _replace_routing_block(text: str, block: str, label: str) -> str:
    start = text.find(ROUTING_BEGIN)
    end = text.find(ROUTING_END)
    if start == -1 or end == -1 or end < start:
        raise HarnessError(
            f"{label}: missing routing markers; expected {ROUTING_BEGIN} before {ROUTING_END}"
        )
    return text[:start] + block + text[end + len(ROUTING_END) :]


def _surface_projection_texts(state: RepositoryState) -> dict[str, str]:
    """Expected content of each surface that carries a generated routing block."""
    rows = _routing_rows(state)
    expected: dict[str, str] = {}
    for relative, style in SURFACE_ROUTING_TARGETS:
        path = state.repo / relative
        if not path.is_file():
            raise HarnessError(f"missing routing surface: {relative}")
        current = path.read_text(encoding="utf-8")
        expected[relative] = _replace_routing_block(
            current, _render_routing_block(rows, style), relative
        )
    return expected
...
def _validate_projection(state: RepositoryState) -> list[str]:
    ...
    try:
        surfaces = _surface_projection_texts(state)
    except HarnessError as exc:
        errors.append(str(exc))
        return errors
    for name, expected in sorted(surfaces.items()):
        if (state.repo / name).read_text(encoding="utf-8") != expected:
            errors.append(f"{name}: generated routing block is stale; run: aikb refresh")
    return errors
...
def command_refresh(args: argparse.Namespace) -> int:
    ...
    targets.extend(sorted(_surface_projection_texts(state).items()))
    stale = [
        name
